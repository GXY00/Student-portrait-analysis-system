        ...
        <div v-if="currentView === 'dashboard'" class="dashboard-content">
          <div class="dashboard-header">
            <div style="display: flex; align-items: center;">
              ...
              <button
                v-if="currentDimension === 'student' || (currentDimension === 'class' && ['admin', 'teacher'].includes(userRole))"
                @click="generateTags"
                class="generate-btn"
              ...
              <div v-if="currentDimension === 'student'" class="student-view-container">
                ...
                <div class="student-tags-wrapper" :class="{'full-width': userRole === 'student'}">
                  ...
                  <div v-else class="tags-grid-wrapper">
                    <!-- 画像雷达图 -->
                    <div v-if="tags.length > 0" class="portrait-chart-card">
                      <div class="chart-header">
                        <h4>综合能力雷达图</h4>
                      </div>
                      <div ref="portraitRadarChart" class="chart-body"></div>
                    </div>
...
export default {
  ...
  watch: {
    ...
    currentDimension (val) {
      if (val === 'student') {
        ...
        if (['admin', 'teacher'].includes(this.userRole)) {
          this.fetchClassList().then(() => {
            if (this.selectedClass) {
              this.fetchClassTags(this.selectedClass.class_id)
            }
          })
        }
      }
  ...
  methods: {
    handleResize () {
      ...
      if (this.classRadarChartInstance) {
        this.classRadarChartInstance.resize()
      }
    },
    updateTime () {
      const now = new Date()
    ...
    async generateTags () {
      ...
      try {
        ...
        // 判断当前维度
        if (this.currentDimension === 'class') {
          // 班级维度
          url = 'http://localhost:5000/api/v1/tags/class/generate'
          if (!this.selectedClass) {
            alert('请先选择一个班级')
            this.loadingTags = false
            return
          }
          payload.class_id = this.selectedClass.class_id
        } else {
          // 学生维度
          if (['teacher', 'admin'].includes(this.userRole)) {
            if (!this.selectedStudent) {
    ...
    initPortraitChart () {
      const dom = this.$refs.portraitRadarChart
      if (!dom || this.tags.length === 0) return
      ...
      // 提取数值型标签用于绘制雷达图
      const numericTags = this.tags.filter(tag => {
        const val = parseFloat(tag.tag_value)
        return !isNaN(val) && tag.tag_type !== '基础信息' // 排除非数值和基础信息
      })

      // 如果没有足够的数值标签，尝试映射一些常见的等级标签
      if (numericTags.length < 3) {
        // 简单处理：如果没有足够数据，不显示图表或显示空状态
        // 这里为了演示，我们假设总是尝试绘制，如果数据不足则显示提示
      ...
      // 选取前6个最重要的标签（或全部）
      const chartTags = numericTags.slice(0, 8)
      ...
      chartTags.forEach(tag => {
        ...
        let max = 10 // 默认最大值设为10，适应大部分1-10的指标
          ...
          data: [
            {
              ...
              areaStyle: {
                color: new echarts.graphic.RadialGradient(0.1, 0.6, 1, [
                  { color: 'rgba(96, 91, 236, 0.1)', offset: 0 },
                  { color: 'rgba(96, 91, 236, 0.7)', offset: 1 }
                ])
              }
            }
          ]
