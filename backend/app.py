...
def generate_tags():
    ...
    try:
        ...
        if ok:
            log_operation(username or 'system', f'生成了{s_name}的学生画像', 1, user_id)
            return jsonify({"success": True, "msg": "标签生成成功"}), 200
        else:
            log_operation(username or 'system', f'生成{s_name}的学生画像失败', 0, user_id)
            return jsonify({"success": False, "msg": f"标签生成失败: {msg}"}), 500
...
def get_teacher_students():
    ...
    try:
        ...
        # 3. 查询学生
        sql = """
            SELECT s.student_id, s.stu_name as name, s.student_no, c.class_name
            FROM student s
            LEFT JOIN class c ON s.class_id = c.class_id
        """
        params = []
        ...
        # 如果有班级限制 (教师且已分配班级)
        if class_id:
            sql += " WHERE s.class_id = %s"
            params.append(class_id)
        elif role_id == 2:
            # 教师若无班级，不应该看到任何学生
             return jsonify({"success": True, "data": []}), 200
             
        # 管理员可以看到所有学生，或者如果未来有其他逻辑可以在这里扩展
...
def summaryAI():
    ...
    words = (
        """请作为一位优秀的教育专家，根据以下学生画像数据，结合具体的理论知识、教育心理学原理和学习方法，为该学生生成一份详细的总结和建议，具体格式为：
        1.学习行为与认知风格：分析学习习惯（如专注度、偏好模式），推荐适配的资源形式与时间管理策略。  
        2.知识掌握与能力缺口：定位具体知识盲区，生成“措题变式”为主的精准补救路径，避免题海战术。  
        3.心理特征与非智力因素：评估动机与抗压能力，提供激发内区力的目标设定建议及针对性沟通话术。 
        最后加上200字左右总结
        以下是具体数据，返回内容必须严格遵守我给你的标准格式，适当使用markdown但绝对不要使用表格\n\n""" + fileContent
    )
